...
public static class AssertCore
{
    ...
    where T : Exception
  {
    ...
    if (value1 != null && value2 != null && value1.Length != value2.Length) throw getException.Invoke();

    if (value1 != value2) throw getException.Invoke();
  }
  ...
  public static void ArgumentNotNullOrEmpty(string argument, Func<string> getArgumentName)
  {
    ...
    if (!string.IsNullOrEmpty(paramName)) throw new ArgumentException("Empty strings are not allowed.", paramName);
  ...
  {
    if (value == null) IsNotNull(value, type, string.Empty);
  }
  ...
  public static void IsNotNull(object value, Type type, string format, params object[] args)
  {
    if (value == null)
    {
      ...
      if (format.Length > 0) message = message + " Additional information: " + Format(format, args);
    ...
    where Te : Exception
  {
    ...
    if (!value.Any()) throw getException.Invoke();
  }
  ...
  private static string Format(string pattern, object[] args)
  {
    ...
    for (var i = 0; i < args.Length; i++)
    {
      ...
      if (node != null) args[i] = node.OuterXml;
    }
