...
namespace EmitMapper.MappingConfiguration
{
    public class DefaultMapConfig : MapConfigBase<DefaultMapConfig>
    {
        ...
        */
        ...
        private readonly List<string> _shallowCopyMembers = new List<string>();
        private readonly List<string> _deepCopyMembers = new List<string>();
        ...
        private static readonly DefaultMapConfig _instance;
 ...
 => _instance;
        ...
        public override IRootMappingOperation GetRootMappingOperation(Type from, Type to)
        {
            ...
            After:
                        var res = base.GetRootMappingOperation(from, to);
            */
            IRootMappingOperation res = base.GetRootMappingOperation(from, to);
        ...
        public override string GetConfigurationName()
        {
            ...
            After:
                        var configName = base.GetConfigurationName() +
            */
            string configName = base.GetConfigurationName() +
                new[]
                {
        ...
        )
        {
            ...
            After:
                        var tp = new TypesPair(from, to);
            */
            TypesPair tp = new TypesPair(from, to);
            ...
            foreach (MemberInfo toMi in toMembers)
            {
                if (toMi.MemberType == MemberTypes.Property)
                {
                    ...
                    After:
                                        var setMethod = ((PropertyInfo)toMi).GetSetMethod();
                    */
                    MethodInfo setMethod = ((PropertyInfo)toMi).GetSetMethod();
                ...
                if (fromMi.MemberType == MemberTypes.Property)
                {
                    ...
                    After:
                                        var getMethod = ((PropertyInfo)fromMi).GetGetMethod();
                    */
                    MethodInfo getMethod = ((PropertyInfo)fromMi).GetGetMethod();
                    if (getMethod == null)
                    {
                        continue;
                    }
                ...
                After:
                                var op = CreateMappingOperation(processedTypes, fromRoot, toRoot, toPath, fromPath, fromMi, toMi);
                */
                IMappingOperation op = CreateMappingOperation(processedTypes, fromRoot, toRoot, toPath, fromPath, fromMi, toMi);
                if (op != null)
                {
                    result.Add(op);
                }
            ...
            )
        {
            ...
            After:
                        ...
                        var origSrcMemberDescr = new MemberDescriptor(fromPath.Concat(new[] { fromMi }).ToArray());
            */
            MemberDescriptor origDestMemberDescr = new MemberDescriptor(toPath.Concat(new[] { toMi }).ToArray());
            MemberDescriptor origSrcMemberDescr = new MemberDescriptor(fromPath.Concat(new[] { fromMi }).ToArray());
            ...
            After:
                        ...
                        var srcMemberDescr = new MemberDescriptor(fromPath.Concat(new[] { fromMi }).ToArray());
            */
            MemberDescriptor destMemberDescr = new MemberDescriptor(toPath.Concat(new[] { toMi }).ToArray());
            MemberDescriptor srcMemberDescr = new MemberDescriptor(fromPath.Concat(new[] { fromMi }).ToArray());
