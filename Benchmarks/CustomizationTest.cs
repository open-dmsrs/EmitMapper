...
public static class CustomizationTest
{
  ...
  public static void Initialize()
  {
    _emitMapper = ObjectMapperManager.DefaultInstance.GetMapper<B2, A2>(
      new DefaultMapConfig().ConstructBy(() => new A2.Int(0)).NullSubstitution<decimal?, decimal>(state => 42)
        .NullSubstitution<bool?, bool>(state => true).NullSubstitution<int?, int>(state => 42)
        .NullSubstitution<long?, long>(state => 42).ConvertUsing<long, int>(value => (int)value + 1)
        .ConvertUsing<short, int>(value => value + 1).ConvertUsing<byte, int>(value => value + 1)
        .ConvertUsing<decimal, int>(value => (int)value + 1).ConvertUsing<float, int>(value => (int)value + 1)
      ...
      cfg =>
        {
          ...
          cfg.CreateMap<B2, A2>().ForMember(s => s.Nullable1, opt => opt.NullSubstitute((decimal)42))
            .ForMember(s => s.Nullable2, opt => opt.NullSubstitute(true))
            .ForMember(s => s.Nullable3, opt => opt.NullSubstitute(42)).ForMember(
              s => s.Nullable4,
              opt => opt.NullSubstitute((long)42));
        });
